...
import { findProvider, isCryptoProvider, normalizeProvider } from '../../lib/providers';
import { COLORS } from '../../lib/portfolio';

export default function ImportationsPage() {
  const { state, commit, notify } = usePortfolio();
  const router = useRouter();
  const params = useSearchParams();
  const targetAccount = state.accounts.find(a => a.id === params.get('compte'));
  ...
  const mapped = useMemo(() => csv ? mapCsv(csv, mapping, { provider: normalizeProvider(source), account: accountName.trim(), currency, kind }) : undefined, [csv, mapping, source, accountName, currency, kind]);
  useEffect(() => {
    if (targetAccount) { setSource(targetAccount.provider); setAccountName(targetAccount.name); setCurrency(targetAccount.currency); setKind(isCryptoProvider(targetAccount.provider) ? 'Crypto' : 'FNB'); }
  }, [targetAccount]);
  ...
  const valid = !!preview?.positions.length && !mapped?.errors.length && rateValid && (!csv || !!source.trim() && (!!accountName.trim() || !!mapping.account));
  ...
  async function loadFile(file: File) {
    ...
    try {
      if (/\.pdf$/i.test(file.name)) {
        if (!findProvider(source)?.pdf) throw Error('La lecture PDF est disponible uniquement pour les relevés Disnat. Pour cet établissement, utilisez le modèle CSV de positions ou la saisie manuelle.');
        ...
        if (id !== request.current) return;
        setCsv(result); setShowMapping(false); setMapping(Object.fromEntries(Object.entries(ALIASES).map(([key, aliases]) => [key, result.headers.find(h => aliases.includes(normalize(h))) || ''])));
      }
  ...
  function confirm() {
    ...
    next.imports = [{ id: uid(), name, source: normalizeProvider(source), date: new Date().toISOString(), count: preview.positions.length, asOf: statement?.asOf }, ...state.imports];
  ...
  return <>
    ...
    <ProviderCatalog source={source} disabled={loading} onSelect={value => { setSource(value); setAccountName(''); setKind(isCryptoProvider(value) ? 'Crypto' : 'FNB'); reset(); }} />
    <div className="provider-settings"><ProviderField value={source} disabled={loading || !!statement || !!backup} onChange={value => { setSource(value); setKind(isCryptoProvider(value) ? 'Crypto' : 'FNB'); }} /><p className="form-note">Votre établissement n’est pas dans la liste? Saisissez son nom. Le modèle CSV Folio et la saisie manuelle restent disponibles.</p></div>
    <section className="import-workspace">
      ...
      <div className={'dropzone ' + (drag ? 'dragging' : '') + (loading ? ' loading' : '')} onDragOver={e => { e.preventDefault(); setDrag(true); }} onDragLeave={() => setDrag(false)} onDrop={e => { e.preventDefault(); setDrag(false); if (e.dataTransfer.files[0]) void loadFile(e.dataTransfer.files[0]); }}>
        ...
        <input ref={fileInput} className="sr-only" type="file" accept={findProvider(source)?.pdf ? '.pdf,.csv,.json' : '.csv,.json'} aria-label="Fichier à importer" onChange={e => { if (e.target.files?.[0]) void loadFile(e.target.files[0]); }} />
      </div>
      ...
      <p className="form-note">Importez des positions actuelles, pas un historique d’achats et de ventes. Les CSV natifs peuvent nécessiter une adaptation au modèle Folio. Seuls les prix et coûts en CAD ou USD sont pris en charge.</p>
      {csv && <div className="form-grid import-defaults"><label className="field"><span>Compte par défaut *</span><input value={accountName} onChange={e => setAccountName(e.target.value)} placeholder="CELI, REER, CELIAPP, REEE, non enregistré…" maxLength={100} list="import-accounts" /><datalist id="import-accounts">{state.accounts.filter(a => normalizeProvider(a.provider) === normalizeProvider(source)).map(a => <option key={a.id} value={a.name}>{a.currency}</option>)}</datalist></label><label className="field"><span>Devise par défaut</span><select value={currency} onChange={e => setCurrency(e.target.value as Currency)}><option>CAD</option><option>USD</option></select></label><label className="field"><span>Catégorie par défaut</span><select value={kind} onChange={e => setKind(e.target.value as AssetKind)}>{Object.keys(COLORS).map(value => <option key={value}>{value}</option>)}</select></label><p className="form-note">Les colonnes Compte, Établissement, Devise et Catégorie du fichier remplacent ces valeurs lorsqu’elles sont renseignées. Vérifiez les comptes ci-dessous avant de confirmer.</p></div>}
      {csv && <>{(!mappingComplete || showMapping) && <><h3 className="mapping-title">Correspondance des colonnes</h3><div className="mapping">{Object.entries(LABELS).map(([key, label]) => <label className="field" key={key}><span>{label}</span><select value={mapping[key] || ''} onChange={e => setMapping({ ...mapping, [key]: e.target.value })}><option value="">{label.endsWith('*') ? 'Choisir une colonne' : 'Valeur par défaut'}</option>{csv.headers.map(h => <option key={h}>{h}</option>)}</select></label>)}</div></>}{mappingComplete && !showMapping && <div className="mapping-confirmed"><CheckCircle2 size={16} /><span>Colonnes reconnues · vérifiez l’aperçu</span><button type="button" className="text-button" onClick={() => setShowMapping(true)}><SlidersHorizontal size={13} />Modifier</button></div>}{hasUsd && !statement && <label className="field import-rate"><span>Taux USD → CAD *</span><input type="number" min="0.01" max="100" step="any" value={rate} onChange={e => setRate(e.target.value)} placeholder="1 USD en CAD" /></label>}{mapped?.errors.length ? <p className="error" role="alert">{mapped.errors.slice(0, 5).join('\n')}{mapped.errors.length > 5 ? '\nEt ' + (mapped.errors.length - 5) + ' autres erreurs.' : ''}</p> : null}</>}
