...
export default function PreferencesPage() {
  ...
  function reset() { if (commit({ ...EMPTY_PORTFOLIO })) { notify('Le portefeuille a été vidé.'); router.push('/importations'); } }
  return <><PageHeading title="Préférences" subtitle="Les paramètres de votre espace personnel." actions={false} /><div className="settings-form"><div className="setting-row"><div><h3>Devise de référence</h3><p>Affichage consolidé en dollars canadiens</p></div><span className="tag">CAD</span></div><form className="setting-row" onSubmit={saveRate}><div><h3>Taux de change USD → CAD</h3><p>Taux manuel ou extrait du dernier relevé PDF.<br />Les gains de change historiques ne sont pas calculés.</p></div><div className="rate-control"><label className="field"><span>1 USD en CAD</span><input key={state.usdCad} name="rate" type="number" min="0.01" max="100" step="any" required defaultValue={state.usdCad} /></label><button className="button" type="submit"><Check />Enregistrer</button></div></form><div className="setting-row"><div><h3>Sauvegarde complète</h3><p>Comptes, positions et historique du portefeuille</p></div><button className="button" onClick={backup}><Download />Sauvegarder</button></div><div className="setting-row"><div><h3>Restaurer une sauvegarde</h3><p>Fichier Folio au format JSON</p></div><Link className="button" href="/importations"><Upload />Restaurer</Link></div><div className="setting-row"><div><h3>Stockage des données</h3><p>Sauvegarde en ligne dans votre compte.<br />Retrouvez votre portefeuille sur vos autres appareils.</p></div><span className="tag">En ligne</span></div><div className="setting-row"><div><h3>Effacer le portefeuille</h3><p>Supprime les comptes et leurs données de votre espace en ligne.</p></div><button className="icon-button" onClick={() => setResetting(true)} aria-label="Effacer le portefeuille" title="Effacer le portefeuille"><Trash2 /></button></div></div>
