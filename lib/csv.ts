...
export const ALIASES: Record<string, string[]> = { symbol: ['symbole', 'symbol', 'ticker'], name: ['nom', 'name', 'description'], kind: ['categorie', 'category', 'type'], quantity: ['quantite', 'quantity', 'qty', 'shares', 'balance'], cost: ['coutunitaire', 'coutmoyen', 'averagecost', 'unitcost', 'cost'], price: ['prixunitaire', 'prix', 'price', 'marketprice'], currency: ['devise', 'currency'], account: ['compte', 'account'], provider: ['etablissement', 'provider', 'source'], costCurrency: ['deviseducout', 'costcurrency'], bookValue: ['valeurcomptable', 'bookvalue'], marketValue: ['valeurmarchande', 'marketvalue'], valuationCurrency: ['devisedesvaleurs', 'valuationcurrency'], asOf: ['date', 'asof'] };
...
export function readCsv(raw: string): CsvData {
  ...
  if (headers.some(h => ['incurrency', 'outcurrency', 'transactionid', 'txid'].includes(normalize(h))) && !headers.some(h => ALIASES.quantity.includes(normalize(h)))) throw Error('Ce fichier est un historique de transactions. Cet import nécessite des positions consolidées avec quantités et prix.');
...
export function mapCsv(data: CsvData, map: Record<string, string>, defaults: { provider: string; account: string; currency: Currency; kind: AssetKind }) {
  ...
  for (const [index, raw] of data.rows.entries()) {
    ...
    const provider = read('provider') || defaults.provider, name = read('account') || defaults.account, currency = (read('currency') || defaults.currency).toUpperCase() as Currency;
    const accountCurrency = (read('valuationCurrency') || currency) as Currency;
    ...
    if (read('bookValue') || read('marketValue')) {
      ...
      p.valuationCurrency = read('valuationCurrency') as Currency;
    ...
    const key = [provider, name, currency, p.symbol].join('|');
